...
async def run_stream(dut, ops, read_full, p_valid=100, p_ready=100,
    ...
    while results_done < len(ops):
        ...
        # In gate-level sim (UNIT_DELAY=#1) combinational outputs settle one
        # timestep AFTER the inputs change, so reading at zero-time samples
        # stale handshake values and the transfer prediction skews (only shows
        # up when valid/ready toggle -> the back-pressure test). Wait most of
        # the clock period so the #1 gate chains have rippled before sampling.
...
# ---------------------------------------------------------------------------
# Directed test for the two new direct ops: NEG and COPYSIGN.
#
# These are bit-level sign operations (no arithmetic datapath), so the golden
# model in ../Golden_model/fp8_math.py is the reference. We drive every case
# through the streaming interface in FULL mode and check result/flags/exc.
#   NEG      : opcode 12 (0x0C)  -> negate(A)      = A with sign flipped
#   COPYSIGN : opcode 13 (0x0D)  -> copySign(A,B)  = |A| bits with B's sign
# ---------------------------------------------------------------------------
def _import_golden():
    import sys
    gm = os.path.join(os.path.dirname(__file__), "..", "Golden_model")
    if gm not in sys.path:
        sys.path.insert(0, gm)
    from fp8_math import fp8_math
    from fp8_common import OP_NEG, OP_COPYSIGN
    return fp8_math, OP_NEG, OP_COPYSIGN


@cocotb.test()
async def test_neg_copysign(dut):
    """Exhaustive NEG + broad COPYSIGN against the golden model (FULL mode)."""
    fp8_math, OP_NEG, OP_COPYSIGN = _import_golden()
    cocotb.start_soon(Clock(dut.clk, 10, units="ns").start())
    await reset_dut(dut)

    vectors = []
    # NEG: all 256 inputs (B is don't-care; rm irrelevant).
    for a in range(256):
        res, fl, ex = fp8_math(a, 0x00, OP_NEG, 0)
        vectors.append((a, 0x00, OP_NEG, 0, res, fl, ex))
    # COPYSIGN: all A x {+, -, +normal, -normal} sign donors in B.
    for b in (0x00, 0x80, 0x38, 0xB8):
        for a in range(256):
            res, fl, ex = fp8_math(a, b, OP_COPYSIGN, 0)
            vectors.append((a, b, OP_COPYSIGN, 0, res, fl, ex))

    dut._log.info(f"NEG+COPYSIGN: driving {len(vectors)} directed cases")
    await run_stream(dut, build_ops_full(vectors), read_full=True,
                     p_valid=100, p_ready=100)
    dut._log.info(f"NEG+COPYSIGN: all {len(vectors)} cases passed")
