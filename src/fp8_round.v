...
//   mant_wide   – NRM_MW bits: [MW-1]=hidden, [MW-2:MW-4]=mant3,
//                 [MW-5]=guard, [MW-6]=round, [MW-7:0]=sticky
...
module fp8_round (
    ...
    input  wire [`NRM_MW-1:0] mant_wide,
    input  wire              is_zero,
    ...
    reg [MW-1:0] wsr;               // mant_wide deslocada (subnormal)
    reg        lost;
    // wsr[MW-1] nunca e' lido (apos o shift subnormal e' sempre 0); sink p/ linter.
    wire _unused_rnd = &{1'b0, wsr[MW-1]};
    ...
    always @(*) begin
        ...
        // ---------------- SUBNORMAL ----------------
            ...
            if (d_sub >= MW[6:0]) begin
                // tudo descartado -> sticky decide arredondamento p/ menor sub
                lost = |mant_wide;
                wsr  = {MW{1'b0}};
                if (lost) wsr[0] = 1'b1;
            end else begin
                lost = |(mant_wide & (({{(MW-1){1'b0}},1'b1} << d_sub) - {{(MW-1){1'b0}},1'b1}));
                wsr  = mant_wide >> d_sub;
