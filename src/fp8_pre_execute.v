...
module fp8_pre_execute (
    ...
    // NOTE: A, B, rounding_mode[3] e alguns bits de flag entram pela interface
    // comum do pipeline mas nao sao usados aqui; sink explicito mais abaixo.
    ...
    // Sinais que chegam pela interface comum mas nao sao usados neste estagio.
    wire _unused_pre = &{1'b0, A, B, rounding_mode[3],
                         flagsA[6:5], flagsA[2], flagsB[6:5], flagsB[2]};
    ...
    always @(*) begin
        ...
        // ==============================================================
        // SQRT — operacao UNARIA sobre A (B ignorado). Tratada a' parte.
        //   NaN      -> propaga (quiet), sem NV
        //   +Inf     -> +Inf ;  -Inf -> NaN + NV
        //   +-0      -> +-0
        ...
        if (opcode == `OPCODE_CVT_I2F || opcode == `OPCODE_CVT_U2F) begin
            ...
            if (a_is_nan) begin
                ...
                if (signA) begin
                    ...
                    special_result = {1'b0, 4'b1111, 3'b000};
                    special_flags[`FLAG_INF] = 1'b1;
                end
            end else if (a_is_zero) begin
                special_result = {signA, 7'b000_0000};
                special_flags[`FLAG_ZERO] = 1'b1;
            end else if (signA) begin
                special_result = CANONICAL_NAN;
                special_flags[`FLAG_NAN]  = 1'b1;
                special_flags[`FLAG_QNAN] = 1'b1;
                special_exceptions[`EXC_INVALID_OP] = 1'b1;
            end else begin
                use_special = 1'b0;   // positivo finito -> iterativo
            end
        end

        // ==============================================================
        // SCALB — A * 2^n (n = B como int8). Unaria em A p/ os especiais;
        //   NaN -> propaga ; +-Inf -> +-Inf ; +-0 -> +-0 ; finito -> round.
        // ==============================================================
        else if (opcode == `OPCODE_SCALB) begin
            if (a_is_nan) begin
                special_result = {signA, 4'b1111, mantA};
                special_flags[`FLAG_NAN]  = 1'b1;
                special_flags[`FLAG_QNAN] = 1'b1;
                ...
                use_special = 1'b0;   // finito -> regua + round
            end
        end
